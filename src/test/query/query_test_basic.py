...
def run():
    ...
    for i in range(NUM_TESTS):
        ...
        # 启动服务端
        print("Starting server for test", i + 1, "...")
        ...
        for key, value in ansDict.items():
            if value != 0:
                match = False
                if value > 0:
                    print('In basic query test' + str(i + 1) + ', Mismatch, your answer lacks: ' + key)
                else:
                    print('In basic query test' + str(i + 1) + ', Mismatch, your answer has extra: ' + key)

        if match:
            score += SCORES[i]
            print("Test", i + 1, "PASSED (score:", SCORES[i], ")")
        else:
